...
def get_clients_this_round(fed_args, round):
    ...
    elif fed_args.num_clients < fed_args.sample_clients:
        clients_this_round = list(range(fed_args.num_clients))
    elif not fed_args.balance_sample:
        random.seed(round)
        clients_this_round = sorted(random.sample(range(fed_args.num_clients), fed_args.sample_clients))
    else:
        random.seed(round)
        clients_this_round = []
        base_sample = fed_args.sample_clients // fed_args.num_datasets
        remainder_id = random.sample(range(fed_args.num_datasets), fed_args.sample_clients % fed_args.num_datasets)
        for i in range(fed_args.num_datasets):
            client_num_ds = base_sample + (1 if i in remainder_id else 0)
            client_id_ds = random.sample(range(fed_args.num_clients_dataset * i, fed_args.num_clients_dataset * (i + 1)), client_num_ds)
            clients_this_round.extend(sorted(client_id_ds))
